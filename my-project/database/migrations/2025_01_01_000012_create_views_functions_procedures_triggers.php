    ...
    public function up(): void
    {
        ...
        DB::unprepared("
            ...
            BEGIN
                ...
                IF fn_is_bike_available(p_bike_id) = 0 THEN
                    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Bicycle is not available for rental.';
                END IF;

                INSERT INTO borrower (full_name, id_photo_id, registered_at)
                VALUES (p_full_name, p_id_photo_ref, v_now);
                SET v_borrower_id = LAST_INSERT_ID();

                SET v_base_fee = fn_compute_rental_fee(p_duration_hrs);

                INSERT INTO rental (borrower_id, bike_id, staff_id, start_time, rental_duration_hrs, base_fee, overtime_fee, total_fee, status)
                VALUES (v_borrower_id, p_bike_id, p_staff_id, v_now, p_duration_hrs, v_base_fee, 0.00, v_base_fee, 'active');
                SET p_rental_id = LAST_INSERT_ID();

                INSERT INTO payment (rental_id, amount_paid, payment_status, paid_at)
                VALUES (p_rental_id, p_amount_paid, 'pending', v_now);

                UPDATE bicycle SET status = 'rented' WHERE bike_id = p_bike_id;

                INSERT INTO activity_log (user_id, user_type, action, timestamp, details)
                VALUES (p_staff_id, 'staff', 'OPEN_RENTAL', v_now,
                    CONCAT('Rental #', p_rental_id, ' opened for borrower: ', p_full_name,
                           ' | Bike ID: ', p_bike_id, ' | Duration: ', p_duration_hrs, ' hrs | Base Fee: ', v_base_fee));
            END
        ");
        ...
        DB::unprepared("
            ...
            BEGIN
                ...
                SELECT bike_id, borrower_id, start_time, rental_duration_hrs, base_fee
                INTO   v_bike_id, v_borrower_id, v_start_time, v_agreed_hrs, v_base_fee
                FROM   rental WHERE rental_id = p_rental_id AND status = 'active';

                IF v_bike_id IS NULL THEN
                    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'No active rental found with that ID.';
                END IF;

                SET v_actual_hrs   = ROUND(TIMESTAMPDIFF(MINUTE, v_start_time, v_now) / 60.0, 4);
                SET v_overtime_hrs = v_actual_hrs - v_agreed_hrs;

                IF v_overtime_hrs > 0 THEN
                    SET v_penalty_fee = fn_compute_overtime_penalty(v_overtime_hrs);
                    INSERT INTO overtime (rental_id, overtime_hrs, penalty_fee, recorded_at)
                    VALUES (p_rental_id, v_overtime_hrs, v_penalty_fee, v_now);
                END IF;

                SET v_total_fee = v_base_fee + v_penalty_fee;

                UPDATE rental SET end_time = v_now, rental_duration_hrs = v_actual_hrs,
                    overtime_fee = v_penalty_fee, total_fee = v_total_fee, status = 'completed'
                WHERE rental_id = p_rental_id;

                SELECT amount_paid INTO v_existing_paid FROM payment WHERE rental_id = p_rental_id LIMIT 1;

                UPDATE payment SET amount_paid = v_existing_paid + p_extra_payment,
                    payment_status = 'settled', paid_at = v_now
                WHERE rental_id = p_rental_id;

                UPDATE bicycle  SET status = 'available' WHERE bike_id = v_bike_id;
                UPDATE borrower SET id_photo_id = NULL WHERE borrower_id = v_borrower_id;

                INSERT INTO activity_log (user_id, user_type, action, timestamp, details)
                VALUES (p_staff_id, 'staff', 'CLOSE_RENTAL', v_now,
                    CONCAT('Rental #', p_rental_id, ' closed | Actual hrs: ', v_actual_hrs,
                           ' | Overtime hrs: ', GREATEST(v_overtime_hrs, 0),
                           ' | Penalty: ', v_penalty_fee, ' | Total Fee: ', v_total_fee));
            END
        ");
        ...
        DB::unprepared("
            ...
            BEGIN
                ...
                SELECT status, `condition` INTO v_old_status, v_old_condition
                FROM bicycle WHERE bike_id = p_bike_id;

                UPDATE bicycle SET status = p_status, `condition` = p_condition WHERE bike_id = p_bike_id;

                INSERT INTO activity_log (user_id, user_type, action, timestamp, details)
                VALUES (p_user_id, p_user_type, 'UPDATE_BIKE_STATUS', NOW(),
                    CONCAT('Bike ID: ', p_bike_id, ' | Status: ', v_old_status, ' → ', p_status,
                           ' | Condition: ', v_old_condition, ' → ', p_condition));
            END
        ");
        ...
        DB::unprepared("
            ...
            BEGIN
                IF p_action = 'add' THEN
                    INSERT INTO staff (admin_id, username, password_hash, full_name, role, status)
                    VALUES (p_admin_id, p_username, p_password_hash, p_full_name, p_role, 'active');
                    SET p_out_staff_id = LAST_INSERT_ID();
                    INSERT INTO activity_log (user_id, user_type, action, timestamp, details)
                    VALUES (p_admin_id, 'admin', 'ADD_STAFF', NOW(),
                        CONCAT('Added staff: ', p_full_name, ' (', p_username, ') | Role: ', p_role));

                ELSEIF p_action = 'update' THEN
                    UPDATE staff SET full_name = p_full_name,
                        password_hash = IF(p_password_hash IS NOT NULL, p_password_hash, password_hash),
                        role = p_role, status = p_status
                    WHERE staff_id = p_staff_id;
                    SET p_out_staff_id = p_staff_id;
                    INSERT INTO activity_log (user_id, user_type, action, timestamp, details)
                    VALUES (p_admin_id, 'admin', 'UPDATE_STAFF', NOW(),
                        CONCAT('Updated staff ID: ', p_staff_id, ' | Name: ', p_full_name,
                               ' | Role: ', p_role, ' | Status: ', p_status));

                ELSEIF p_action = 'deactivate' THEN
                    UPDATE staff SET status = 'inactive' WHERE staff_id = p_staff_id;
                    SET p_out_staff_id = p_staff_id;
                    INSERT INTO activity_log (user_id, user_type, action, timestamp, details)
                    VALUES (p_admin_id, 'admin', 'DEACTIVATE_STAFF', NOW(),
                        CONCAT('Deactivated staff ID: ', p_staff_id));

                ELSE
                    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Invalid action. Use: add | update | deactivate';
                END IF;
            END
        ");
        ...
        DB::unprepared("
            ...
            BEGIN
                ...
                SELECT status INTO v_bike_status FROM bicycle WHERE bike_id = NEW.bike_id;
                IF v_bike_status != 'available' THEN
                    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Cannot create rental: bicycle is not available.';
                END IF;
            END
        ");
        ...
        DB::unprepared("
            ...
            BEGIN
                UPDATE bicycle SET status = 'rented' WHERE bike_id = NEW.bike_id;
            END
        ");
        ...
        DB::unprepared("
            ...
            BEGIN
                UPDATE rental SET overtime_fee = NEW.penalty_fee, total_fee = base_fee + NEW.penalty_fee
